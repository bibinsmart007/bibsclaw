...
import { readFile, writeFile, listFiles, executeCommand } from "./tools.js";
...
You do NOT take autonomous actions, post content, send messages, make purchases, 
or modify any systems without Bibin explicitly asking you to do so in each message.
...
export const AGENTS: Record<string, AgentDefinition> = {
  ...
  cto: {
    id: "cto", name: "CTO Agent", icon: "🧠", department: "Leadership",
...
TOOLS YOU USE: web_search to check API status pages, fetch_url to check endpoints,
read_file/list_files to review code, execute_command for system checks.

Always provide structured technical reports with: Status, Issues Found, Recommendations.`
  },

  finance: {
    id: "finance", name: "Finance Agent", icon: "💰", department: "Leadership",
...
Always give Bibin clear numbers: Current spend, Projected monthly, Budget remaining, ROI.`
  ...
  social: {
    id: "social", name: "Social Accounts Agent", icon: "📱", department: "AISocialGrowth",
    model: "claude-haiku-4-5",
...
Always recommend actions for Bibin to approve before anything is done.`
  ...
  store: {
    id: "store", name: "Store Integration Agent", icon: "🛒", department: "AISocialGrowth",
    model: "claude-haiku-4-5",
...
Never access payment or sensitive customer data.`
  ...
  products: {
    id: "products", name: "Winning Products Agent", icon: "🏆", department: "AISocialGrowth",
    model: "claude-sonnet-4-5",
...
USE web_search to find: TikTok trending products, Amazon bestsellers, Google Trends data,
Reddit discussions about products, YouTube product review trends.

Format output as a ranked list with scores and reasoning for each product.`
  },

  competitor: {
    id: "competitor", name: "Competitor Intel Agent", icon: "🔍", department: "AISocialGrowth",
    model: "claude-sonnet-4-5",
...
Deliver structured intelligence reports: Who they are, What's working for them,
Their weaknesses, Opportunities for AISocialGrowth to differentiate.`
  },

  content: {
    id: "content", name: "AI Content Studio Agent", icon: "✍️", department: "AISocialGrowth",
    model: "claude-sonnet-4-5",
...
Never publish anything — only draft for Bibin's approval.`
  ...
  video: {
    id: "video", name: "Video Script Agent", icon: "🎬", department: "AISocialGrowth",
    model: "claude-sonnet-4-5",
...
Always provide: Hook, Script, Shot suggestions, Caption, Hashtags. Multiple variations.`
  ...
  calendar: {
    id: "calendar", name: "Content Calendar Agent", icon: "📅", department: "AISocialGrowth",
    model: "claude-haiku-4-5",
...
Always output calendar as a structured table with: Date, Platform, Content Type, Topic, Status.`
  ...
  analytics: {
    id: "analytics", name: "Analytics Agent", icon: "📈", department: "AISocialGrowth",
    model: "claude-haiku-4-5",
...
When Bibin shares data, analyse it and return: What's working, What's not, What to do next.`
  ...
  adperf: {
    id: "adperf", name: "Ad Performance Agent", icon: "🎯", department: "AISocialGrowth",
    model: "claude-sonnet-4-5",
...
Never make changes to live campaigns — only recommend. Bibin approves all changes.`
  ...
  reports: {
    id: "reports", name: "Client Reports Agent", icon: "📋", department: "AISocialGrowth",
    model: "claude-sonnet-4-5",
...
Always make the client feel the value. Lead with wins, address challenges with solutions.`
  ...
  funnel: {
    id: "funnel", name: "Sales Funnel Agent", icon: "🚀", department: "AISocialGrowth",
    model: "claude-haiku-4-5",
...
For each stage identify: Current conversion rate, Industry benchmark, Gap, Fix.`
  ...
  marketing: {
    id: "marketing", name: "Marketing Agent", icon: "📣", department: "Growth Ops",
    model: "claude-sonnet-4-5",
...
Always draft content for Bibin's review before any posting.`
  ...
  sales: {
    id: "sales", name: "Sales Agent", icon: "💼", department: "Growth Ops",
    model: "claude-sonnet-4-5",
...
Always personalise every outreach. Research the prospect first. Draft for Bibin's approval.`
  ...
  geo: {
    id: "geo", name: "GEO Agent", icon: "🌐", department: "Growth Ops",
    model: "claude-sonnet-4-5",
...
GEO is the practice of optimising content so that AI systems like ChatGPT, Perplexity, 
Claude, Gemini, and Copilot cite and recommend your brand in their generated responses.
...
Search for what AI engines are currently saying about our brand and competitors.`
  ...
  seo: {
    id: "seo", name: "SEO Agent", icon: "🔎", department: "Growth Ops",
    model: "claude-sonnet-4-5",
...
Always provide: Keyword, Monthly Volume (estimate), Difficulty, Intent, Content angle.`
  ...
  research: {
    id: "research", name: "Research Agent", icon: "🧪", department: "Growth Ops",
    model: "claude-sonnet-4-5",
...
Always cite sources. Flag when information may be outdated.`
  ...
  dev: {
    id: "dev", name: "Dev Agent", icon: "💻", department: "Engineering",
    model: "claude-sonnet-4-5",
...
TECH STACK: TypeScript, Node.js, Express, Socket.IO, Anthropic SDK, Telegram grammy,
...
Wait for Bibin's approval before suggesting to commit anything.`
  ...
  debug: {
    id: "debug", name: "Debug Agent", icon: "🐛", department: "Engineering",
    model: "claude-haiku-4-5",
...
When given an error, always ask: What is the error? What was happening when it occurred?
What does the relevant code look like? What has already been tried?`
  },

  qa: {
    id: "qa", name: "QA Agent", icon: "✅", department: "Engineering",
    model: "claude-haiku-4-5",
...
When asked to test something, produce: Test cases, Expected results, Pass/Fail status,
...
async function runTool(toolName: string, input: Record<string, string>): Promise<string> {
  try {
    switch (toolName) {
      ...
      case "list_files": {
        const result = await listFiles(input.path || ".");
        return result.success ? result.output : `Error: ${result.error}`;
      }
